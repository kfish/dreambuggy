...
simpleTerrain2D side =
    terrain2D side (fromArray (Array.repeat (side * side) 1.0))
...
terrain2D side arr0 =
    let
        ...
        sizesRanges =
            unfoldWhile (\( x, r ) -> ( x // 2, r * h )) (\( x, r ) -> x > 1) ( side, range )
    in
        List.foldl (allQuads side) arr0 sizesRanges
...
allQuads side ( size, range ) arr0 =
    let
        coords =
            List.concat <| subSquares size side
    in
        List.foldl (quad side size range) arr0 coords
...
quad side len range ( x, y ) arr =
    let
        ...
        bR =
            getXY (x + len) (y + len) 0 arr

        l2 =
            len // 2

        cR =
            range * getXY (x + l2) (y + l2) 0 arr

        nR =
            range * getXY (x + l2) y 0 arr

        eR =
            range * getXY x (y + l2) 0 arr

        wR =
            range * getXY (x + len) (y + l2) 0 arr

        sR =
            range * getXY (x + l2) (y + len) 0 arr
    in
